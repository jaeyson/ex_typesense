defmodule StopwordsTest do
  ...
  setup_all do
    ...
    on_exit(fn ->
      ...
      |> Enum.each(fn stopword ->
        stop_id = stopword.id
        {:ok, _} = ExTypesense.delete_stopword(stop_id)
      end)
    end)
  ...
  test "success: get stopword", %{conn: conn, map_conn: map_conn} do
    ...
    case ExTypesense.get_stopword(stop_id) do
      {:ok, %StopwordsSetRetrieveSchema{stopwords: %{id: id}}} ->
        assert stop_id === id
  ...
  test "success: delete stopword", %{conn: conn, map_conn: map_conn} do
    ...
    case ExTypesense.delete_stopword(stop_id) do
      {:ok, map} when is_map(map) ->
        assert {:ok, map} === {:ok, %{"id" => stop_id}}

      {:ok, struct} when is_struct(struct) ->
        assert {:ok, struct} === {:ok, %OpenApiTypesense.Stopwords{id: stop_id}}
    end
