defmodule CollectionTest do
  ...
  test "success: clone collection", %{schema: schema, conn: conn, map_conn: map_conn} do
    ...
    assert {:ok, %CollectionResponse{name: "new_x"}} = ExTypesense.clone_collection(name, "new_x")
    ...
    assert {:ok, %CollectionResponse{name: "prod_x"}} =
             ExTypesense.clone_collection(Product, "prod_x")
  ...
  test "success: dropping collection deletes all documents", %{schema: schema} do
    ...
    assert {:error, %ApiResponse{message: "Not Found"}} = ExTypesense.get_document(schema.name, 1)
  end
  ...
  test "success: get collection name by alias", %{schema: schema, conn: conn, map_conn: map_conn} do
    ...
    assert nil === ExTypesense.get_collection_name("xyz")
    assert name === ExTypesense.get_collection_name(alias_name)
    assert name === ExTypesense.get_collection_name(conn, alias_name)
    assert name === ExTypesense.get_collection_name(map_conn, alias_name)
  end
