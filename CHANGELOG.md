...
* `ExTypesense.upsert_search_synonym/4` in favor of `upsert_synonym_set/3` or `upsert_synonym_set_item/4` when using Typesense v30.0+.
* `ExTypesense.get_search_synonyms/2` in favor of `retrieve_synonym_set_items/2` or `retrieve_synonym_sets/1` when using Typesense v30.0+.
* `ExTypesense.get_search_synonym/3` in favor of `retrieve_synonym_set/2` or `retrieve_synonym_set_item/3` when using Typesense v30.0+.
* `ExTypesense.delete_search_synonym/3` in favor of `delete_synonym_set/2` or `delete_synonym_set_item/3` when using Typesense v30.0+.
